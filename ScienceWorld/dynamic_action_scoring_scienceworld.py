...
from data_utils import add_current_place, add_current_objects, sanitizeStr, formalize_action
from data_utils import compose_instance_v4
from eval_utils import load_model, findValidActionNew, findValidActionList, findValidActionOne, load_variation, get_model_output, get_model_output_withscore, findValidActionWithSystem2, getFilteredValidActions, sbert_search, clean_look, is_action_failed 
...
def sanitizeObservation(obsIn, infoIn):
    obs = infoIn['taskDesc'] + " OBSERVATION " + obsIn # observation包含任务描述
    return obs
        ...
        while not done:           
            ...
            if action is None: 
                ...
                if args.slow_agent:                    
                    ...
                    for a in generate_action_lst:
                        p = cal_prob(a, input_str, lm_model, tokenizer, device, args)
                        prob_scores.append(float(p))
                    # predStrs, prob_scores = get_model_output_withscore(args, input_str, tokenizer, lm_model, device, logger)
                    # print("LLM output is:")
                    # print(prob_scores)
                    ob = "[OBSERVATION]: "+obs
                    # q_values = IQLmodel.get_q(clean(task_description),clean(info["look"]), clean(info['inv']), clean(ob), predStrs)
                    q_values = IQLmodel.get_q(clean(task_description),clean(info["look"]), clean(info['inv']), clean(ob), generate_action_lst)
                    # print(q_values)
                    logger.info(f"p values :{prob_scores}")
                    logger.info(f"q values :{q_values}")
                    action_index = decide_action(prob_scores,list(q_values),step,args)
                    # action = findValidActionOne(predStrs[action_index], env, info['look'], recent_actions, sbert_model, logger)
                    action = generate_action_lst[action_index]
                    logger.info(f"final action choose :{action}")
...
def parse_args():
    ...
    parser.add_argument("--lm_path", default="IL_model/checkpoint-8000") 
    parser.add_argument("--simplification_str", default="easy")
    parser.add_argument("--beams", type=int, default=6)
    parser.add_argument("--max_episode_per_file", type=int, default=5000)
    parser.add_argument("--mode", default="fast_system")
    parser.add_argument("--set", default="test_mini")
    parser.add_argument("--output_path", default="fast_logs/test8000-iqlonly")
    ...
    parser.add_argument("--demo_file", default="demos.json", type=str)
    ...
    parser.add_argument("--iql_path", type=str, default="IQL/IQLmodel/final_iql_scienceworld.pt")
