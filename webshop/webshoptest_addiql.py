...
def llm_tgi(prompt, tokenizer, model):
    ...
    # Generate
    generate_input = {
        "input_ids": inputs.input_ids,  # prompt，输入部分
        "do_sample": False,  # 是否采样，相当于每次不再是选择top k(beam num)个概率最大的，而是加了一些采样
        "top_k": 50,
        "top_p": 0.90,
        "temperature": 0.5,  # 采样温度，较高的值如0.8会使输出更加随机，而较低的值如0.2则会使其输出更具有确定性
        "num_beams": 1,  # 当搜索策略为束搜索（beam search）时，该参数为在束搜索（beam search）中所使用的束个数，当num_beams=1时，实际上就是贪心搜索（greedy decoding）
...
def evaluate_web_llama3finetuned_addIQL(env, model, tokenizer, IQLmodel, sbert_model, args):
    ...
    for r in range(1):
        ...
        while step < args['env_step_limit']:
            ...
            # print(responses)
            # 得到当前状态的available actions列表，这个根据不同环境要写不同的函数哈
            avaliable_actions = get_available_actions(env.get_available_actions())
            # 将response映射到合法动作空间，不用改直接用就行
            ...
            if len(mapped_actions)>1:
                ...
                # decide_action函数会根据步数和两个值列表来得到最终选择action的序号，从mapped动作里面取出最后决定的action
...
def parse_args():
    ...
    parser.add_argument("--iql_path", type=str, default="./IQLsrc/final_iql_webshop_twin_20.pt")
...
def main():
    ...
    #     api_key="7b34cba1021a400980b527d1c1b49626",
