...
from .value_functions import DRRN_Q, DRRN_V, DRRNS_Q, DRRNS_V, DRRN_Q_web, DRRN_V_web, DRRN_TwinQ
...
class ImplicitQLearning_S(nn.Module):
    def __init__(self, args, optimizer_factory):
        super().__init__()
        self.sp = spm.SentencePieceProcessor()
        self.sp.Load(args.spm_path)
        self.qf = DRRNS_Q(len(self.sp), args.embedding_dim, args.hidden_dim).to(DEFAULT_DEVICE)
        self.q_target = copy.deepcopy(self.qf).requires_grad_(False).to(DEFAULT_DEVICE)
        self.vf = DRRNS_V(len(self.sp), args.embedding_dim, args.hidden_dim).to(DEFAULT_DEVICE)
        self.v_optimizer = optimizer_factory(self.vf.parameters())
        self.q_optimizer = optimizer_factory(self.qf.parameters())
        self.tau = args.tau
        self.beta = args.beta
    ...
    def update(self,observations, freelook, inv,  actions, next_observations, next_look, next_inv, rewards, terminals):
        obs_ids = [self.sp.EncodeAsIds(o) for o in observations]
        free_ids = [self.sp.EncodeAsIds(f) for f in freelook]
        inv_ids = [self.sp.EncodeAsIds(i) for i in inv]
        # TextWorld
        ...
        with torch.no_grad():
            target_q = self.q_target(free_ids, inv_ids, obs_ids, action_ids)
            next_v = self.vf(nextfree_ids, nextinv_ids, nextobs_ids)

        # v, next_v = compute_batched(self.vf, [observations, next_observations])

        # Update value function
        v = self.vf(free_ids, inv_ids, obs_ids)
        adv = target_q - v
        v_loss = asymmetric_l2_loss(adv, self.tau)
        self.v_optimizer.zero_grad(set_to_none=True)
        v_loss.backward()
        ...
        # Update Q function
        targets = rewards + (1. - terminals.float()) * self.discount * next_v.detach()
        qs = self.qf(free_ids, inv_ids, obs_ids, action_ids)
        # print(qs.shape)
        # print(targets.shape)
    ...
    def act(self, freelook, inv, observation, actions):
        free_ids = [self.sp.EncodeAsIds(freelook) for i in range(len(actions))]
        inv_ids = [self.sp.EncodeAsIds(inv) for i in range(len(actions))]
        obs_ids = [self.sp.EncodeAsIds(observation) for i in range(len(actions))]
        action_ids = [self.sp.EncodeAsIds(action) for action in actions]
        q_value = list(self.q_target(free_ids, inv_ids, obs_ids, action_ids))
        lst = [i for i in zip(q_value, actions)]
        lst.sort(reverse=True)
        return lst[0][1]
